...
http://average-coder.blogspot.hk/2011/09/simple-socks5-server-in-c.html
*/
...
bool handle_handshake(int sock, char *buffer) {
	...
	{
		printf("false-- read_size != sizeof(MethodIdentificationPacket) || packet.version != 5 ");
        return false;
	}		
    if(recv_sock(sock, buffer, packet.nmethods) != packet.nmethods)
	{	printf("recv_sock(sock, buffer, packet.nmethods) != packet.nmethods\n");	
        return false;
	}	
	...
	{
		printf("handle_handshake ret1 %d\n", false);
        return false;
	}
    ret=(response.method == METHOD_AUTH) ? check_auth(sock) : true;
	printf("handle_handshake ret %d\n", ret);
	return ret;
}
...
/*****************dt  begin**********************/			
			...
			ssInput.buf_len = (uint32_t)recvd;
			ssInput.conn = conn;
			ssInput.send_sock = send_sock;
